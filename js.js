...
<body>
    ...
    <section class="card reveal" id="projects-section">
      ...
      <div id="project-list" class="projects-grid">
        ...
        <div class="project-card">
          ...
          <div style="margin-top: 12px; display: flex; gap: 10px; flex-wrap: wrap;">
            <button class="btn ghost" onclick="downloadTP2Blend()">📁 Télécharger TP2.blend</button>
  ...
  // TP2 avec fallback
  function downloadTP2File(url, filename){
      const link=document.createElement('a');
      link.href=url;
      link.download=filename;
      link.style.display='none';
      ...
      setTimeout(()=>{
          if(!confirm(`Le téléchargement de "${filename}" a-t-il démarré ? Si non, cliquez OK pour ouvrir le lien.`)) return;
  ...
  document.addEventListener('DOMContentLoaded',function(){ setupActions(); revealOnScroll(); appendStoredProjects(); window.addEventListener('scroll',revealOnScroll); console.log('🚀 Portfolio chargé — Téléchargements activés'); });
  </script>
</body>
</html>
